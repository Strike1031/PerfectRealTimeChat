...
const NewDiscussBody = ({ closeChildDialog }) => {
  ...
  // Create discuss chat
  const createDiscussChat = async () => {
    if (!groupInfo) return;
    const { chatDisplayPic, chatName, users } = groupInfo;

    if (!chatName) return displayWarning("Please Enter a Discuss Name");
    ...
    try {
      ...
      const { data } = await axios.post("/api/chat/discuss", formData, config);
  ...
  return (
    <div>
      ...
      <section className="newGroup d-flex position-relative mb-4">
        ...
        />
        <CustomTooltip
          title="Edit Discuss Display Pic"
          placement="top-start"
          arrow
        >
          <i
      ...
      <section className={`${formFieldClassName}`}>
        <label htmlFor="groupName" className={`${formLabelClassName}`}>
          Discuss Name <span className="required">*</span>
        </label>
        <input
          type="text"
          value={chatName}
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter") createDiscussChat();
          }}
          required
          autoFocus
          name="groupname"
