...
const NewTeamBody = ({ closeChildDialog }) => {
  ...
  const createTeamChat = async () => {
    ...
    if (!chatName) return displayWarning("Please Enter a Team Name");
    ...
    try {
      ...
      const { data } = await axios.post("/api/chat/teams", formData, config);
